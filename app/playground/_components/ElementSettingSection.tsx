...
function ElementSettingSection({ selectedEl, clearSelection }: Props) {
    ...
    return (
        <div className='w-96 shadow p-4 space-y-4 overflow-auto h-[90vh] rounded-xl mt-2 mr-2 bg-popover text-popover-foreground border border-border'>
            <h2 className='flex gap-2 items-center font-bold text-foreground'>
                <SwatchBook /> Settings
            </h2>
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <label className='text-sm block text-muted-foreground'>Text Color</label>
                    <input type='color'
                        className='w-10 h-10 rounded-lg mt-1 border border-border'
            ...
            <div>
                ...
                <ToggleGroup
                    ...
                    className="bg-input/60 dark:bg-input/30 rounded-lg p-1 inline-flex w-full justify-between border border-border"
                >
                    <ToggleGroupItem value="left" className="p-2 rounded hover:bg-accent/10 dark:hover:bg-accent/20 flex-1">
                        <AlignLeft size={20} />
                    </ToggleGroupItem>
                    <ToggleGroupItem value="center" className="p-2 rounded hover:bg-accent/10 dark:hover:bg-accent/20 flex-1">
                        <AlignCenter size={20} />
                    </ToggleGroupItem>
                    <ToggleGroupItem value="right" className="p-2 rounded hover:bg-accent/10 dark:hover:bg-accent/20 flex-1">
                        <AlignRight size={20} />
                    </ToggleGroupItem>
                </ToggleGroup>
            </div>
            ...
            <div className="flex items-center gap-4">
                <div>
                    <label className='text-sm block text-muted-foreground'>Background</label>
                    <input type='color'
                        className='w-10 h-10 rounded-lg mt-1 border border-border'
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {classes.length > 0 ? (
                        classes.map((cls) => (
                            <span
                                key={cls}
                                className="flex text-xs items-center gap-1 px-2 py-1 text-sm rounded-full bg-muted/10 border border-border text-muted-foreground"
                            >
                                {cls}
                                <button
                        ...
                        <span className="text-muted-foreground text-sm">No classes applied</span>
                    )}
                </div>
